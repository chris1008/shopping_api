...
class Auth:
    ...
    login_period = 60*60*24*3 # seconds of 3 days
    ...
    def encode_refresh_token(self, user_id, email):
        ...
        self.rds.set(email, token, ex=self.login_period)
        return token

    def refresh_token(self, refresh_token):
        try:
            payload = jwt.decode(refresh_token, self.JWT_SECRET, algorithms=[self.JWT_ALGORITHM], options={"verify_exp": False},)
            if (payload['scope'] == 'refresh_token'):
                server_record = self.rds.get(payload['sub'])
                if server_record == None: 
                    raise HTTPException(status_code=401, detail='Refresh token expired')
                email = payload['sub']
                user_id = payload['id']
                new_token = self.encode_token(user_id, email)
                self.rds.set(email, refresh_token, ex=self.login_period)
                return new_token
